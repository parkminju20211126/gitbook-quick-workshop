...
채팅 입력창의 `+` → **Quick knowledge** → **Choose a folder** → `./ops-data/` 를 선택합니다.

`./ops-data/` 폴더에는 다음 파일이 들어 있습니다.

- `ops-billing.csv` — 6월/7월 서비스별 비용·사용량
- `ops-monitoring.csv` — 7월 일별 모니터링 지표
- `ops-resources.csv` — 리소스 인벤토리
- `ops-findings.md` — Health/Trusted Advisor 특이사항 + 내부 포탈 티켓
